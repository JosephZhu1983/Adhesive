<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ViewConfigItemList.aspx.cs" Inherits="Adhesive.Config.Web.ViewConfigItemList" %>
...
<body>
    ...
    <asp:Button runat="server" Text="上一级"  ID="btnGotoParent" style="height:25px;width:65px;BACKGROUND-IMAGE: url(../Images/button-3.gif); BORDER-TOP-STYLE: none; BACKGROUND-REPEAT: no-repeat; BORDER-RIGHT-STYLE: none; BORDER-LEFT-STYLE: none; BORDER-BOTTOM-STYLE: none"
     ...
     &nbsp;
     <asp:Button runat="server" Text="根节点"  ID="btnGotoRoot" style="height:25px;width:65px;BACKGROUND-IMAGE: url(../Images/button-3.gif); BORDER-TOP-STYLE: none; BACKGROUND-REPEAT: no-repeat; BORDER-RIGHT-STYLE: none; BORDER-LEFT-STYLE: none; BORDER-BOTTOM-STYLE: none"
        onclick="btnGotoRoot_Click"/>
        &nbsp;
<asp:Button ID="btnAdd" runat="server" Text="增加" onclick="btnAdd_Click" style="height:25px;width:52px;BACKGROUND-IMAGE: url(../Images/button-2.gif); BORDER-TOP-STYLE: none; BACKGROUND-REPEAT: no-repeat; BORDER-RIGHT-STYLE: none; BORDER-LEFT-STYLE: none; BORDER-BOTTOM-STYLE: none"/>
    ...
    <tr>
    <td colspan="7" >
    <asp:DataGrid ID="dgConfigItems" runat="server" AutoGenerateColumns="False"
            HorizontalAlign="Left" Width="85%" CellPadding = "3" 
            onitemdatabound="dgConfigItems_ItemDataBound">
        <Columns>
        <asp:TemplateColumn>
            <ItemTemplate>
            <asp:HyperLink ID="HyperLink1" runat="server" NavigateUrl='<%#string.Format("ViewConfigItemList.aspx?appName={0}&parentPathItemNames={1}&prevParentId={2}&parentId={3}",Eval("AppName"),CombinePathItemNames(Eval("Name").ToString()),Eval("ParentId"),Eval("Id")) %>' Text="下一级"></asp:HyperLink>
            </ItemTemplate>
                <ItemStyle HorizontalAlign="Center"  Wrap="false"/>
            </asp:TemplateColumn>
            <asp:TemplateColumn HeaderText="名称">
            <ItemTemplate>
            <asp:HyperLink ID="HyperLink2" runat="server" NavigateUrl='<%#string.Format("EditConfigItem.aspx?appName={0}&parentPathItemNames={1}&id={2}&parentId={3}&type=edit",Eval("AppName"),GetParentPathItemNames(),Eval("Id"),Eval("ParentId")) %>' Text='<%#Eval("Name") %>' ToolTip="点击链接进行编辑操作"></asp:HyperLink>
            </ItemTemplate>
