...
namespace Adhesive.ComponentPerformance.Core
{
    internal class Memcached
    {
        ...
        { 
            ...
            { "limit_maxbytes", new ComponentItem("其它", "容量", ItemValueType.TextValue) },

            { "rusage_user", new ComponentItem("CPU", "CPU用户时间", ItemValueType.TotalValue) },
            { "rusage_system", new ComponentItem("CPU", "CPU系统时间", ItemValueType.TotalValue) },

            { "curr_connections", new ComponentItem("连接", "当前连接数", ItemValueType.StateValue) },
            { "total_connections", new ComponentItem("连接", "连接数", ItemValueType.TotalValue) },


            { "bytes_read", new ComponentItem("网络", "网络读取字节数", ItemValueType.TotalValue) },
            { "bytes_written", new ComponentItem("网络", "网络发送字节数", ItemValueType.TotalValue) },
            ...
            { "total_items", new ComponentItem("数据", "存储的内容变化数量", ItemValueType.TotalValue) },
            { "bytes", new ComponentItem("数据", "当前存储内容所占用的字节数", ItemValueType.StateValue) },
